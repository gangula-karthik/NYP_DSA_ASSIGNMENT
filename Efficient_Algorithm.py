...
def two_number_sum(seq: List[int], z: int) -> str:
    ...
    """
    # initializing the high and low pointers
    low = 0
    high = len(seq) - 1
    # doing a while loop until the pointers meet each other
    while low < high:
        curr_sum = seq[low] + seq[high]
        # If the sum is equal to the target, return the two numbers
        if curr_sum == z: 
            return f"x = {seq[low]}\ny = {seq[high]}\n{seq[low] + seq[high] == z}"
        # If the sum is less than the target, move the low pointer up
        elif curr_sum < z:
            low += 1
        # If the sum is more than the target, move the high pointer down
        else: 
            high -= 1
    # If the pointers meet each other and no sum is found, return not found
